(function() {
  ...
  exports.Simulator = Simulator = (function() {
    ...
    Simulator.prototype.follow = function(caption) {
      ...
      return this.goto(target[1]);
    };

    Simulator.prototype.swipe_card = function(name) {
      var card, key, parser, tform, value, _base, _name;
      card = CARDS[name.toLowerCase()] || (function() {
      ...
      if (tform = this.state.screen.element.first('tform')) {
        if (parser = tform.first('card', {
          ...
          for (key in card) {
            value = card[key];
            (_base = this.state.variables)[_name = "card." + key] || (_base[_name] = {
              type: "string",
              value: null
            });
