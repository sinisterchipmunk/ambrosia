(function() {
  ...
  exports.Simulator = Simulator = (function() {
    ...
    Simulator.prototype.goto = function(id) {
      var display, match, parser, print, screen, tform, _base;
      if (match = /^tmlvar:(.*)$/.exec(id)) {
        id = id.replace(match[0], this.find_variable(match[1]).value);
      }
      ...
      if (!(screen.tags.length > 0)) {
        throw new Error("Screen element '" + id + "' cannot be empty");
      }
      ...
      if (print = this.state.screen.element.first('print')) {
        ...
        this.state.print += this.process_output_element(print);
      }
      if (tform = this.state.screen.element.first('tform')) {
        if (parser = tform.first('card', {
          ...
          return this.perform_risk_management();
        }
      }
    };

    Simulator.prototype.validate_screen_variants = function() {
      var key, variant, variants, _i, _len, _results;
      variants = this.state.screen.element.search('variant');
      _results = [];
      for (_i = 0, _len = variants.length; _i < _len; _i++) {
        variant = variants[_i];
        if (key = variant.attrs.key) {
          if (__indexOf.call(KEYS, key) < 0) {
            throw new Error("Variant key '" + key + "' is not valid");
          }
          if (key === 'cancel') {
            throw new Error("'cancel' is not allowed as a variant key");
          } else {
    ...
    Simulator.prototype.fail = function(message) {
      var econn;
      if (message == null) message = "unknown failure";
      if (this.at_submit_screen()) {
        econn = this.state.screen.element.first('submit').attrs.econn;
        this.evaluate(this.find_variable('err.description'), 'string', {
          lo: message
        });
        this.goto(econn);
        return this.start();
    ...
    Simulator.prototype.perform_risk_management = function() {
      var data, name, pan;
      pan = this.find_variable('card.pan').value;
      for (name in CARDS) {
        data = CARDS[name];
        if (data.pan === pan) {
          this.find_variable('card.scheme').value = data.scheme;
          this.find_variable('card.parser.verdict').value = 'online';
          return;
        }
      }
    ...
    Simulator.prototype.process_output_element = function(e) {
      var attr, attrs, key, match, result, str, sub, value, _i, _len, _ref2, _ref3;
      str = "" + ((function() {
      ...
      while (match = /<getvar(.*?)\/?\s*>/.exec(str)) {
        ...
        result = this.find_variable(attrs['name']).value;
    ...
    Simulator.prototype.find_variable = function(varname) {
      ...
      if (!variable) throw new Error("Variable not defined: " + varname);
      return variable;
    };
    ...
    Simulator.prototype.process_form_submission = function(submit_element) {
      ...
      if (!submit_element.attrs.econn) {
        throw new Error("Submit tag in screen '" + this.state.screen.id + "' requires an 'econn' attribute");
      }
      ...
      for (_i = 0, _len = getvars.length; _i < _len; _i++) {
        ...
        _results.push(this.state.post[variable_name] = this.find_variable(variable_name).value);
      }
      return _results;
    };
    ...
    Simulator.prototype.find_possible_variants = function() {
      ...
      if (this.state.key === 'cancel' && this.state.screen.element.attrs.cancel) {
        return [this.state.screen.element.attrs.cancel];
      }
      if (next) {
        _ref2 = next.all("variant");
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
    ...
    Simulator.prototype.fill_in = function(field, content) {
      ...
      return this.find_variable(field).value = content;
    };

    Simulator.prototype.swipe_card = function(name) {
      var key, parser, tform, value, _ref2;
      ...
      if (tform = this.state.screen.element.first('tform')) {
        if (parser = tform.first('card', {
          ...
          for (key in _ref2) {
            value = _ref2[key];
            this.find_variable("card." + key).value = value;
          }
          return this.start();
        }
      }
